...
void line_down(t_data *data, int x, int y)
{
  ...
  i = -1;
  data->bres->error = (float)data->bres->dx / 2;
  //printf("LINE_DOWN\nx : '%d'\ty : '%d'\tdx : '%d'\tdy : '%d'\txincr : '%d'\tyincr : '%d'\terror : '%f'\n", x, y, data->bres->dx, data->bres->dy, data->bres->xincr, data->bres->yincr, data->bres->error);
  while (++i < data->bres->dx)
  {
    x += data->bres->xincr;
    data->bres->error += (float)data->bres->dy;
    if (data->bres->error > data->bres->dx)
    {
      data->bres->error -= (float)data->bres->dx;
      y += data->bres->yincr;
    }
    //printf("put x : '%d'\ty : '%d'\n", x, y);
...
void line_up(t_data *data, int x, int y)
{
  ...
  i = -1;
  //printf("\nLINE_UP\n");
  data->bres->error = (float)data->bres->dy / 2;
...
void ft_bres(t_data *data, int x1, int y1, int x2, int y2)
{
  ...
  printf("x1 : '%d'\ty1 :'%d'\tx2 : '%d'\ty2 : '%d'\tdx : '%d'\tdy : '%d'\n", x1, y1, x2, y2, data->bres->dx, data->bres->dy);
